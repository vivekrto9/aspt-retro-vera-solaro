...
The standalone `/lead-generation-demo` page includes a complete product-interest example for **Northstar One**. The existing home page is unchanged. The demo shows how a derived AstroPages template can turn a product CTA into a privacy-safe lead stored in the canonical `leads.v1` table.
...
| Product context | `kind=contact`, `source=support`, `form_key=northstar-product-interest` |
...
## Adapt it to another product

1. Replace the image in `public/images/` and update the product defaults in `src/data/product-lead-demo.ts`.
2. Change the product name and form key in the endpoint.
