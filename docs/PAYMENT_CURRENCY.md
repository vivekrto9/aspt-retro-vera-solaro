...
Status: **Done locally** on `feature/payment-preference`, based on `develop` at `3df98025aef1c1b2cc0f380d4826d90aa1002a16`.
...
The existing local D1 was copied to `/tmp/vera-payment-preference-d1-before.sqlite` before migration. Its pre-migration Vera counts were 3 services, 15 bookings, and 11 payment attempts. Local migrations `0014`–`0019` applied successfully, preference and all six price columns read back correctly, `PRAGMA foreign_key_check` returned no rows, and a second `pnpm run d1:migrate:local` reported no migrations to apply.

## Verification evidence

- Baseline before implementation: `pnpm run test` — 218/218 passed.
...
- Typecheck: zero errors (three pre-existing unused-value hints).
- Production build: passed.
- Complete repository gate: `pnpm run verify` passed, including 225/225 tests, asset/sales/users/secrets contracts, safety scan, D1 schema contract, Cloudflare runtime contract, typecheck, and build.
...
- A headed Chromium inspection of `/readings/natal-hour` confirmed the preserved Vera layout and responsive rendering. Screenshot: `output/playwright/currency-headed-detail.png`. Expected local availability requests remained provider-blocked because no live Calendly setup was used.
